...
def create_test_frame(width: int, height: int, channels: int, frame_num: int) -> np.ndarray:
    ...
    for y in range(height):
        for x in range(width):
            ...
            if channels > 2:
                frame[y, x, 2] = ((x + y) * 255 // (width + height) + frame_num * 10) % 256
